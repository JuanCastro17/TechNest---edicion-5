...
// Desactivamos el renderizado de advertencias HTML para que ninguna alerta rompa el formato esperado por JS
error_reporting(0);
ini_set('display_errors', 0);

include(__DIR__ . '/../../config/conexion.php');

// TRUCO DE RESPALDO: Si la variable de conexión de config/conexion.php no se cargó, reconectamos directo a tu puerto 3307
if (!isset($conexion) || !$conexion) {
    $conexion = @mysqli_connect("127.0.0.1", "root", "", "technest", 3307);
}

// Si de verdad no hay conexión, devolvemos un JSON de error limpio en vez de un aviso HTML
if (!$conexion) {
...
// Determinar el remitente activo (sea cliente o vendedor)
...
// ACCIÓN A: OBTENER MENSAJES (Se ejecuta en bucle automático)
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $chat_con = isset($_GET['chat_con']) ? intval($_GET['chat_con']) : 0;
    
    // Si es vendedor consultando un cliente, o cliente consultando su propia sala
    $u_id = ($id_vendedor > 0) ? $chat_con : $id_usuario;
    $v_id = ($id_vendedor > 0) ? $id_vendedor : 1; // Asignado por defecto al vendedor 1
...
// ACCIÓN B: ENVIAR MENSAJE (Sin recargar pantalla)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_vendedor > 0) {
        // El remitente es el vendedor corporativo
        ...
        // El remitente es el cliente público
